...
const ProductsPage = props => {
  ...
  const renderPageItems = () => {
    const arrLength =
      store.totalRestaurants !== 0 && store.restaurants.length !== 0 ? Number(store.totalRestaurants) / store.restaurants.length : 3
          ...
          <Fragment>
            ...
            <Pagination className='d-flex justify-content-center'>
              ...
              <PaginationItem
                className='next-item'
                onClick={() => handleNext()}
                disabled={store.params.page === Number(store.totalRestaurants) / store.restaurants.length}
              >
                <PaginationLink href='/' onClick={e => e.preventDefault()}></PaginationLink>
              </PaginationItem>
            </Pagination>
