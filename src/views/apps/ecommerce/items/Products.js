...
const ProductsPage = props => {
  ...
  const renderPageItems = () => {
    const arrLength =
      store.totalItems !== 0 && store.items.length !== 0 ? Number(store.totalItems) / store.items.length : 3
  ...
  const handleNext = () => {
    if (store.params.page !== Number(store.totalItems) / store.items.length) {
          ...
          <Fragment>
            ...
            <Pagination className='d-flex justify-content-center'>
              ...
              <PaginationItem
                className='next-item'
                onClick={() => handleNext()}
                disabled={store.params.page === Number(store.totalItems) / store.items.length}
              >
                <PaginationLink href='/' onClick={e => e.preventDefault()}></PaginationLink>
              </PaginationItem>
            </Pagination>
