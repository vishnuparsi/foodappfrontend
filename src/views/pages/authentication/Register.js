...
const Register = () => {
  ...
  const authentication = () => {
    ...
    authAxios.post('authenticate', user).then(response => {
  ...
  const onSubmit = () => {
    if (isObjEmpty(errors)) {
        const object = { userName: uname, password: pwd, email:mail, status: 'active', restId: 1, role: 'admin'}
        authAxios.post('users/save', object).then(response => { console.log(response) })
        
        setTimeout(authentication, 5000)
    }
